$mfc = (Get-WmiObject -Class "Win32_ComputerSystem" | Select-Object -ExpandProperty Manufacturer).Trim()
#Get-WmiObject Win32_ComputerSystem | Select-Object manufacturer
...
switch -Wildcard ($mfc){
    '*dell*'{
		...
		if ($Result -gt 0) {
			...
			Start-Sleep -s 60
			Get-Content -Path 'C:\Temp\updateOutput.log'
			write-host "Done."
			exit 0
		}
    }
